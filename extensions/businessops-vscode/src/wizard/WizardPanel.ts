...
import { STEPS, Step, questionToStep } from "./steps";
import { WizardState, Question, validateQuestion } from "./schema";
...
async function loadStepsFromYaml(root: string): Promise<Step[]> {
  ...
  if (!wf?.questions?.length) {
    return STEPS; // fallback to hardcoded
  }
  ...
  return steps.length > 0 ? steps : STEPS;
}
...
export class WizardPanel {
  ...
  static async open(extensionUri: vscode.Uri) {
    ...
    // Load steps from YAML workflow (with fallback)
    const steps = await loadStepsFromYaml(root);

    WizardPanel.panel.webview.html = this.renderHtml(extensionUri, lang, initialState, steps);
    ...
    WizardPanel.panel.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === "save") {
        try {
          ...
          WizardPanel.panel?.webview.postMessage({ type: "saveResult", success: true });
          vscode.window.showInformationMessage("BusinessOps Wizard: state saved ✅");
        } catch (err: any) {
          // Notify webview of failure
          WizardPanel.panel?.webview.postMessage({ type: "saveResult", success: false, error: err?.message });
        }
      }
  ...
  private static renderHtml(extensionUri: vscode.Uri, lang: string, initialState: WizardState, steps: Step[]): string {
